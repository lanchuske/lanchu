...
| `lanchu tile [--dry]` | Arranges the agent terminals into a mosaic, strictly the org's own known windows (never a window outside the org). On an odd count, the coordinator's terminal (current lease holder, else a product/supervisor agent) gets a full-height double-width pane. macOS: arranges over the current screen — a dedicated empty Space is not implemented (would need Accessibility-gated keystrokes, which this codebase avoids by design). |
